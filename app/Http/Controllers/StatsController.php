...
class StatsController extends BaseAuthController
{
    public function AdminStats()
    {
         ...
         $stats['tables']['last_terminated_list']=SubscriptionGym::where('status',1)->latest('end_at')->take(5)->get();
         $list=array();
            for ($i = 6; $i > -1; $i--) {
        ...
        for ($i = 12; $i > -1; $i--) {
            $price_of_months[Carbon::now()->subMonth($i)->format('Y-m')] =SubscriptionGym::where(function($q) use($i){
    ...
    public function ManagerStats()
    {
        ...
        $res=new Result();
        $stats=array();
        $stats['cards']['count_employees']=User::whereIn('role',['coach','receptionisite'])->where('gym_id',$gym_id)->count();
        ...
        $stats['tables']['last_add_list']=Subscription::where('status',false)->whereHas('adherent',function ($q) use($gym_id){
            $q->where('gym_id',$gym_id);
        })->latest()->take(5)->get();
